...
output2 = np.loadtxt(f"data/scattering_fit{fitNum2}.out", skiprows=1)
E2 = output2[:,0]*1000 / 1000
phase2 = output2[:,1]
eta2 = output2[:,n_ch+1]


# -------------------------Plot Phase Shift-------------------------
pypl.plot(E, phase, color='blue', linestyle='-'
          , linewidth='1.7', zorder=3.0, label='1 bare state')
pypl.plot(E2, phase2, color='red', linestyle='-', linewidth='1.7'
          , zorder=4.0, label='2 bare states')
...
pypl.axis( [min(E), max(E), -20, max(phaseData)*1.2] )
...
pypl.legend(loc='upper left', numpoints=1
...
pypl.plot(E, eta, color='blue', linestyle='-', linewidth='1.7'
          , zorder=3.0, label='1 bare state')
pypl.plot(E2, eta2, color='red', linestyle='-', linewidth='1.7'
          , zorder=4.0, label='2 bare states')
pypl.ylabel( '$\eta$' )
pypl.xlabel( '$E$ (GeV)' )
# pypl.title(f'S11 {n_bare}b{n_ch}c fit{fitNum}')
# pypl.title(f'S11 {n_bare}b{n_ch}c')
pypl.axis( [min(E), max(E), 0.1, 1.05] )

pypl.legend(loc='lower left', numpoints=1
            , prop={'size': 16}, frameon=False)
