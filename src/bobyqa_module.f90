...
!  * Curtis Abell, July 2022 : slight changes to use DP from kinds.f90
...
!  have been found in the case ```NPT=N+6``` even with more than 100 variables.
    ...
    subroutine bobyqa (n, npt, x, xl, xu, rhobeg, rhoend, iprint, maxfun, calfun)
        ...
        real(DP),dimension(:),intent(in)    :: xl      !! lower bounds on x. The construction of quadratic models
                                                       !! requires XL(I) to be strictly less than XU(I) for each I. Further,
                                                       !! the contribution to a model from changes to the I-th variable is
                                                       !! damaged severely by rounding errors if XU(I)-XL(I) is too small.
        real(DP),dimension(:),intent(in)    :: xu      !! upper bounds on x. The construction of quadratic models
                                                       !! requires XL(I) to be strictly less than XU(I) for each I. Further,
                                                       !! the contribution to a model from changes to the I-th variable is
                                                       !! damaged severely by rounding errors if XU(I)-XL(I) is too small.
        real(DP),intent(in)                 :: rhobeg  !! RHOBEG must be set to the initial value of a trust region radius.
        ...
        real(DP),intent(in)                 :: rhoend  !! RHOEND must be set to the final value of a trust
...
!
90      if (dsq <= 1.0e-3_DP*xoptsq) then
...
!
        if (npt >= n+np) then
            do k = 2 * np, npt
                iw = (real(k-np, DP)-half) / real (n, DP)
                ip = k - np - iw * n
                iq = ip + iw
                if (iq > n) iq = iq - n
                ptsid (k) = real (ip, DP) + real (iq, DP) / real (np, DP) + sfrac
...
!
        ...
        do k = 1, npt
            ...
            if (k == kopt) then
                ...
                iq = real (np, DP) * ptsid (k) - real (ip*np, DP)
                if (iq > 0) then
                    iw = 1
                    if (ip == 0) iw = 2
...
!
        ...
        if (denom <= 1.0e-2_DP*vlmxsq) then
...
260     do kpt = 1, npt
            ...
            pq (kpt) = zero
            ip = ptsid (kpt)
            iq = real (np, DP) * ptsid (kpt) - real (ip*np, DP)
            if (ip > 0) then
                xp = ptsaux (1, ip)
                xpt (kpt, ip) = xp
            end if
...
30      stepsq = zero
        ...
        if (gredsq*delsq <= 1.0e-4_DP*qred*qred) go to 190
!
...
!
        if (stplen < blen) then
            ...
            if (sdec <= 0.01_DP*qred) go to 190
            beta = gredsq / ggsav
            go to 30
        end if
90      crvmin = zero
...
120     iterc = iterc + 1
        ...
        if (temp <= 1.0e-4_DP*qred*qred) go to 190
        temp = sqrt (temp)
        do i = 1, n
            if (xbdi(i) == zero) then
...
!
        ...
        do i = 1, iu
            angt = angbd * real (i, DP) / real (iu, DP)
...
!
        ...
        if (isav < iu) then
            ...
            angt = angbd * (real(isav, DP)+half*temp) / real (iu, DP)
        end if
...
!
        if (sdec > 0.01_DP*qred) go to 120
190     dsq = zero
        do i = 1, n
...
!  component of X to be in the interval [-1,1].
    ...
    subroutine bobyqa_test()
        ...
        real(DP),parameter :: bdl    = - 1.0_DP
        real(DP),parameter :: bdu    = 1.0_DP
        integer,parameter  :: iprint = 2
        integer,parameter  :: maxfun = 500000
        real(DP),parameter :: rhobeg = 1.0e-1_DP
        real(DP),parameter :: rhoend = 1.0e-6_DP
...
30              format (/ / 5 x, '2D output with M =', i4, ',  N =', i4, '  and  NPT =', i4)
                do j = 1, m
                    temp = real (j, DP) * twopi / real (m, DP)
                    x (2*j-1) = cos (temp)
                    x (2*j) = sin (temp)
                end do
        ...
        subroutine calfun (n, x, f)
            ...
            do i = 4, n, 2
                do j = 2, i - 2, 2
                    ...
                    temp = max (temp, 1.0e-6_DP)
                    f = f + 1.0_DP / sqrt (temp)
                end do
            end do
