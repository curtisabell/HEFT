...
for iFit, fitNum in enumerate(fitRange):
    ...
    # ---------------------------Plot this fit--------------------------
    thisPlot, = pypl.plot(m_pi2[:], E_ground[iFit,:], color=colours[iFit],
...
# Legend on plot
pypl.legend(allPlots, fitLeg, loc='lower right', frameon=False, prop={'size': 16})
...
outFile = f'figs/1b2c_Evmpi_multifit_{alphaOrder}slope.{fileType}'
